...
Public Class WebForm1
    ...
    Private Sub ProcessLoginCredentials()
        ...
        Dim err As String
        err += "Start processlogin "
        
        Try
            ''This is for the test site
            'remove before moving to production!
            'If 1 = 1 Then
            '    If UCase(Me.txtUserID.Text) = UCase("test") And txtPassword.Text = "test123" Then
            '        Dim myUserData As Common.UserData
            '        With myUserData
            '            .ID = Me.txtUserID.Text.Trim
            '            .AdminUser = "True" ' XmlData.GetXMLElementValue("AdminUser").ToBoolean
            '            'if admin, automatically make UW
            '            .UWUser = "True" '.AdminUser Or XmlData.GetXMLElementValue("UWUser").ToBoolean
            '            .Accounting = "False" 'XmlData.GetXMLElementValue("Accounting").ToBoolean
            '            .Email = "Testsite@testing.com" 'XmlData.GetXMLElementValue("Email")
            '            .AccountNumber = "11111" 'XmlData.GetXMLElementValue("AccountNumber")
            '            .AgencyName = "Test Agency" 'XmlData.GetXMLElementValue("AgencyName")
            '            .AgentName = "Test Agent" 'XmlData.GetXMLElementValue("AgentName")
            ...
            If ds.Tables(0).Rows.Count = 1 Then
                If ds.Tables(0).Rows(0).Item("StatusText").ToString = "Invalid credentials." Then
        ...
        Catch ex As Exception
            errortrap(err, "Login Screen", ex.Message)
...
UseWeb:
        ...
        Catch ex As Exception
            errortrap("Login Screen Agent", "Login Screen", ex.Message)
            Common.AjaxMsgBoxShow(Me.Page, ex.Message)
            Stop
        Finally
            cn.Close()
    ...
    Private Sub errortrap(ByVal sqlcomm As String, ByVal appsub As String, ByVal errormsg As String)
        ...
        cmd.Parameters.Add("@Page", SqlDbType.VarChar, 50).Value = "Login Screen"
