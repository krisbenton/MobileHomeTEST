...
Public Class quote
    ...
    Public Sub calcAR()
        ...
        ds = runQueryDS("EXEC sp_getARRates '" & territory & "','" & value & "','" & txt_unattstr_AR1.Text.Replace("$", "") & "','" & txt_pp_AR1.Text.Replace("$", "") & "','" & txtEffDate.Text.ToString & "','" & lblquoteNumber.Text & "'", "ColonialMHConnectionString")
        If ds.Tables(0).Rows.Count > 0 Then
        ...
        ds = runQueryDS("EXEC sp_getARRates '" & territory & "','" & value & "','" & txt_unattstr_AR2.Text.Replace("$", "") & "','" & txt_pp_AR2.Text.Replace("$", "") & "','" & txtEffDate.Text.ToString & "','" & lblquoteNumber.Text & "'", "ColonialMHConnectionString")
        If ds.Tables(0).Rows.Count > 0 Then
        ...
        If IsNumeric(txt_unattstr_AR1.Text) And IsNumeric(txtcvgamt.Text) Then
            ...
            If CDbl(txt_unattstr_AR1.Text) > (CDbl(txtcvgamt.Text) * 0.1) Then ' if other structures greater than default (10%) covered amount add.
                ...
                If (FormatCurrency(ar_unattStr1.Text) < FormatCurrency(txt_unattstr_AR1.Text)) Or (FormatCurrency(ar_unattStr1.Text) > (CDbl(txtcvgamt.Text) * 0.1)) Then
                    ...
                    ds = runQueryDS("EXEC sp_getARRates '" & territory & "','" & value & "','" & attdiff & "','" & txt_pp_AR1.Text.Replace("$", "") & "','" & txtEffDate.Text.ToString & "','" & lblquoteNumber.Text & "'", "ColonialMHConnectionString")
                    If ds.Tables(0).Rows.Count > 0 Then
        ...
        If IsNumeric(txt_pp_AR1.Text) And IsNumeric(txtcvgamt.Text) Then
            ...
            If CDbl(txt_pp_AR1.Text) > (CDbl(txtcvgamt.Text) * 0.4) Then ' if personal effects greater than default (40%) covered amount add.
                ...
                If (FormatCurrency(ar_perEff1.Text) < FormatCurrency(txt_pp_AR1.Text)) Or (CDbl(txt_pp_AR1.Text) > (CDbl(txtcvgamt.Text) * 0.4)) Then
                    ...
                    ds = runQueryDS("EXEC sp_getARRates '" & territory & "','" & value & "','" & txt_unattstr_AR1.Text.Replace("$", "") & "','" & ppdiff & "','" & txtEffDate.Text.ToString & "','" & lblquoteNumber.Text & "'", "ColonialMHConnectionString")
                    If ds.Tables(0).Rows.Count > 0 Then
                        'packagePremium = FormatCurrency(ds.Tables(0).Rows(0).Item("PackageRate").ToString)
                        ' packageOtherStructures = FormatCurrency(ds.Tables(0).Rows(0).Item("PackageStructureRate").ToString)
                        packageContents = CInt(FormatCurrency(ds.Tables(0).Rows(0).Item("PackagePersonalPropRate").ToString))
    ...
    Public Sub updatePrimeRateDownPayment(ByVal sender As Object, ByVal e As EventArgs) 'Handles primeRateDPDD.SelectedIndexChanged
        ...
        ModalPopupExtender3.Show()
    End Sub
    Public Sub PremiumFinanceCalc_OptionsChanged(ByVal sender As Object, ByVal e As EventArgs) Handles PremiumFinanceDPQuote.SelectedIndexChanged, PremiumFinanceLenQuote.SelectedIndexChanged

        Dim state As String
        state = lblState.Text
        If state = "NC" Or state = "SC" Or state = "VA" Then 'Colonial Premium Finance
            If PremiumFinanceLenQuote.SelectedIndex < 0 Then
                Exit Sub
            End If
            If PremiumFinanceDPQuote.SelectedIndex < 0 Then
                Exit Sub
            End If
        ...
        Else 'Prime Rate
            ...
            End If
            PrimeRateFinancing()
            PremiumFinanceLenQuote.Visible = False
        End If
    ...
    Protected Sub ColonialPremiumFinancing()
        Dim ds As System.Data.DataSet
        ds = runQueryDS("SELECT ARRateType, state  FROM tblQuotes WHERE quoteID ='" & lblquoteNumber.Text & "'", "ColonialMHConnectionString")
        If ds.Tables(0).Rows.Count > 0 Then
            Dim LloydsState As String = ds.Tables(0).Rows(0).Item("state")
        End If
        Dim downPayment, Length, quoteID As String
        quoteID = lblquoteNumber.Text
        downPayment = PremiumFinanceDPQuote.SelectedItem.Value.ToString
        Length = PremiumFinanceLenQuote.SelectedItem.Value.ToString
        ...
        If ds.Tables(0).Rows.Count > 0 Then
            ...
            lblGeneralFinanceTypeQuote.Text = "Colonial Financing"
            lblGeneralPremiumQuote.Text = FormatCurrency(ds.Tables(0).Rows(0).Item("PremTotal"))
            lblGeneralDownPaymentQuote.Text = FormatCurrency(ds.Tables(0).Rows(0).Item("DownPayment"))
            lblGeneralAmountFinancedQuote.Text = FormatCurrency(ds.Tables(0).Rows(0).Item("AmountFinanced"))
            lblGeneralFinanceChargeQuote.Text = FormatCurrency(ds.Tables(0).Rows(0).Item("FinanceCharge"))
            lblGeneralTotalOfPaymentsQuote.Text = FormatCurrency(ds.Tables(0).Rows(0).Item("TotalOfPayments"))
            lblGeneralMonthlyPaymentQuote.Text = FormatCurrency(ds.Tables(0).Rows(0).Item("MonthlyPayments"))
            lblGeneralAPRQuote.Text = CStr(APR) + "%"
        End If
    ...
    Protected Sub PrimeRateFinancing()
        ...
        If Request.QueryString("quoteID") <> "" Then
            DetailString = fin.CalcFinancingOnlyXML(lblquoteNumber.Text, downPayment)
            If DetailString.Contains("xml") Then
                lblGeneralFinanceTypeQuote.Text = "Prime Rate Financing"
                lblGeneralPremiumQuote.Text = FormatCurrency(DetailString.GetXMLElementValue("Total").ToString)
                lblGeneralDownPaymentQuote.Text = FormatCurrency(DetailString.GetXMLElementValue("DownPayment").ToString)
                lblGeneralAmountFinancedQuote.Text = FormatCurrency(DetailString.GetXMLElementValue("AmountFinanced").ToString)
                lblGeneralFinanceChargeQuote.Text = FormatCurrency(CDec(DetailString.GetXMLElementValue("FinanceCharge").ToString))
                lblGeneralTotalOfPaymentsQuote.Text = DetailString.GetXMLElementValue("NumberOfPayments").ToString
                lblGeneralMonthlyPaymentQuote.Text = DetailString.GetXMLElementValue("Payment").ToString
                lblGeneralAPRQuote.Text = DetailString.GetXMLElementValue("APR").ToString + "%"
            Else
                lblGeneralFinanceTypeQuote.Text = DetailString
            End If
